...
class RobotHandler:
    ...
    def get_wheel_positions(self):
        ...
        wheel_pos[0] = (
            self._right_motors.ReadEncM1(self._addr_right)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_pos[1] = (
            self._left_motors.ReadEncM1(self._addr_left)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_pos[2] = (
            self._right_motors.ReadEncM2(self._addr_right)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_pos[3] = (
            self._left_motors.ReadEncM2(self._addr_left)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        return wheel_pos
    ...
    def get_wheel_velocities(self):
        ...
        wheel_vel[0] = (
            self._right_motors.ReadSpeedM1(self._addr_right)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_vel[1] = (
            self._left_motors.ReadSpeedM1(self._addr_left)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_vel[2] = (
            self._right_motors.ReadSpeedM2(self._addr_right)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        wheel_vel[3] = (
            self._left_motors.ReadSpeedM2(self._addr_left)[1]
            / self._encoder_res
            * self._wheel_circumference
        )
        return wheel_vel
    ...
    def send_command(self, vel_mps):
        ...
        for i in range(len(vel_mps)):
            vel_qpps[i] = int(
                vel_mps[i] / self._wheel_circumference * self._encoder_res
            )
