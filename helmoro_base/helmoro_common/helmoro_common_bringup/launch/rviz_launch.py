...
def generate_launch_description():
    ...
    rviz_config = PathJoinSubstitution(
        [pkg_helmoro_common_bringup, "rviz", "rviz_config.rviz"]
    )
    ...
    uncompress_color_image = ExecuteProcess(
        cmd=[
            ...
            "--ros-args",
            "--remap",
            "in/compressed:=/sensors/camera/color/image_raw/compressed",
            "--remap",
            "out:=/sensors/camera/color/image_raw/uncompressed",
        ],
        output="screen",
    ...
    uncompress_depth = ExecuteProcess(
        cmd=[
            ...
            "--ros-args",
            "--remap",
            "in/compressedDepth:=/sensors/camera/depth/image_raw/compressedDepth",
            "--remap",
            "out:=/sensors/camera/depth/image_raw/uncompressed",
        ],
        output="screen",
        condition=UnlessCondition(LaunchConfiguration("use_sim_time")),
    )
